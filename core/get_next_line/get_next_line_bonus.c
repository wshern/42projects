...
/*   Updated: 2026/09/11 18:12:37 by werlim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd >= FD_MAX || BUFFER_SIZE <= 0)
		return (NULL);
	storage[fd] = update_storage(fd, storage[fd]);
	newline_i = find_newline(storage[fd]);
	rtn_str = gnl_strbefore(storage[fd], newline_i);
	storage[fd] = gnl_strafter(storage[fd], newline_i);
	return (rtn_str);
}
